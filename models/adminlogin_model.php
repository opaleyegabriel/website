...
class Adminlogin_Model extends Model{
    ...
    public function username($data){
      $sql=$this->db->prepare("SELECT username FROM tbl_mywebadmin WHERE username=:username" );
      $sql->execute(array(
     ...
     $sql=$this->db->prepare("SELECT * FROM tbl_mywebadmin WHERE  username=:user AND password=:pwd ");
     $sql->execute(array(
       ':user'=>$data['username'],
       ':pwd'=>$data['password']
       ));
